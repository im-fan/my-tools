...
public abstract class ConsumerAbstract  extends RocketGroupProperty implements CommandLineRunner {

    private Logger logger = LoggerFactory.getLogger(ConsumerAbstract.class);
    ...
    public  void run(String... args){
        ...
        subscribeList();
        consumer.start();
        logger.info("start 消息(普通、定时/延时消息)服务消费者。GroupId(ConsumerId)={}",getGroupId());
    }


    @PreDestroy
    public void shutdown() {
        logger.info("shutdown 消息(普通、定时/延时消息)服务消费者。GroupId(ConsumerId)={}",getGroupId());
        consumer.shutdown();
    }
}
